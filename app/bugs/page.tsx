  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data, error } = await supabase
        .from("bug_reports")
        .select("id, page, body, created_at, user_id")
        .order("created_at", { ascending: false })
        .limit(100);

      if (error) setMessage(error.message);
      else setRows((data as BugRow[]) || []);
      setLoading(false);
    };
    load();
  }, [router]);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 px-4 py-8 pb-28">
      <div className="max-w-md mx-auto">
        ...
        <p className="text-slate-500 text-sm mb-6">{rows.length} reports</p>
          ...
          {rows.map((r) => (
            ...
            >
              <p className="text-xs text-slate-500 mb-1">
                {r.page || "unknown page"} · {timeAgo(r.created_at)}
              </p>
